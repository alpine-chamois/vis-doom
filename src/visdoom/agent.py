...
from visdoom.constants import ENV_ID, HUMAN, MODEL_FOLDER, RGB
...
class Agent:
    ...
    def _make_env(self, render_mode: str) -> Callable[[], Env]:
        ...
        def _env_factory() -> Env:
            ...
            env = ResizeWrapper(
                env, width=self.config.screen_width, height=self.config.screen_height
            )
            # env = GrayscaleWrapper(env)
            env = TransformReward(
                env, lambda r: float(r) * self.config.reward_scale
            )  # Scale reward
            env = Monitor(env)
            return env
    ...
    def train(self) -> None:
        ...
        training_env = self._wrap_vec_env(
            SubprocVecEnv([self._make_env(RGB) for _ in range(self.config.num_envs)])
        )
